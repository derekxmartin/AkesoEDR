/*
 * sentinel-agent/process_table.cpp
...
void
ProcessTable::UpsertFromProcessCtx(const SENTINEL_PROCESS_CTX& ctx)
{
...
void
ProcessTable::OnEvent(const SENTINEL_EVENT& evt)
{
    std::lock_guard<std::mutex> lock(m_mutex);

    /* Handle process creation/termination events from driver */
    if (evt.Source == SentinelSourceDriverProcess) {
...
std::wstring
ProcessTable::GetParentImagePath(const SENTINEL_EVENT& evt)
{
    std::lock_guard<std::mutex> lock(m_mutex);
