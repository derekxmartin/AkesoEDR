/*
 * sentinel-agent/process_table.h
...
#ifndef SENTINEL_PROCESS_TABLE_H
#define SENTINEL_PROCESS_TABLE_H
...
public:
     ...
     */
    void OnEvent(const SENTINEL_EVENT& evt);
     ...
     */
    std::wstring GetParentImagePath(const SENTINEL_EVENT& evt);

private:
    std::unordered_map<ULONG, ProcessEntry> m_entries;
    std::mutex m_mutex;

    void UpsertFromProcessCtx(const SENTINEL_PROCESS_CTX& ctx);
};

#endif /* SENTINEL_PROCESS_TABLE_H */
